...
const compress = async (image, newFileName, effort, quality, resolution) => {
  ...
  const res = await sharp(image.buffer).resize(resolution, resolution, { fit: "outside", withoutEnlargement: true }).avif({ effort, quality }).keepExif().keepIccProfile().toFile(dest);
